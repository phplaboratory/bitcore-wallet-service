...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  if (!_.contains(['livenet', 'testnet','bcclivenet', 'bcctestnet'], opts.network))
...
WalletService.prototype._canCreateAddress = function(ignoreMaxGap, cb) {
  ...
  self.storage.fetchAddresses(self.walletId, function(err, addresses) {
    ...
    console.log("WalletService.prototype._canCreateAddress = "+latestAddresses[0].network)
...
WalletService.prototype._getBlockchainExplorer = function(network) {
  ...
  console.log("WalletService.prototype._getBlockchainExplorer:");
  console.log(opts);
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  if(  addresses[0] && (typeof addresses[0]  === "object")) {
    networkName = addresses[0].network;
    addresses[0]=addresses[0].btcaddress;
    console.log("WalletService.prototype._getUtxos: works");


  } else {
    networkName = Bitcore.Address(addresses[0]).toObject().network;
    console.log("WalletService.prototype._getUtxos: not works");
  }
  console.log(addresses);
  console.log("----------------------------------------");


  console.log("WalletService.prototype._getUtxos: " + networkName );
  var bc = self._getBlockchainExplorer(networkName); //almost correct

  console.log("WalletService.prototype._getUtxos, addresses:");
  console.log(addresses);
...
WalletService.prototype._getUtxosForCurrentWallet = function(addresses, cb) {
  ...
  var allAddresses, allUtxos, utxoIndex,wallet_network;


  self.storage.fetchWallet(self.walletId, function(err, wallet) {
    if (!err && wallet) wallet_network=wallet.network;
  ...
  async.series([
    ...
    function(next) {
      ...
      // console.log("After pluck:");
      // console.log(addressStrs);
      addressStrs[0] = {'btcaddress': addressStrs[0], 'network': wallet_network};
    ...
    self._getActiveAddresses(function(err, activeAddresses) {
      ...
      if (!_.isArray(activeAddresses)) {
        ...
        //   if (!err && wallet) wallet_network = wallet.network;
           ...
           //   activeAddresses[0] = {'btcaddress': activeAddresses[0], 'network': wallet_network};
...
WalletService.prototype._sampleFeeLevels = function(network, points, cb) {
  ...
  console.log("WalletService.prototype._sampleFeeLevels: " + network );
  var bc = self._getBlockchainExplorer(network);
...
WalletService.prototype.getFeeLevels = function(opts, cb) {
  ...
  if (network != 'livenet' && network != 'testnet' && network != 'bcclivenet' && network != 'bcctestnet')
    return cb(new ClientError('Invalid network'));
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    if (!wallet.getNetworkName().endsWith(toAddress.network)) {
...
WalletService.prototype.createTx = function(opts, cb) {
  ...
  console.log("createTx: income request:");
  console.log(opts);

  if (("network" in opts) && opts.network != 'livenet' && opts.network != 'testnet' && opts.network != 'bcclivenet' && opts.network != 'bcctestnet')
    return cb(new ClientError('Invalid network'));
...
WalletService.prototype._broadcastRawTx = function(network, raw, cb) {
  ...
  // console.log("WalletService.prototype._broadcastRawTx: " + network );
  var bc = this._getBlockchainExplorer(network); //correct
...
WalletService.prototype._checkTxInBlockchain = function(txp, cb) {
  ...
  // console.log("WalletService.prototype._checkTxInBlockchain: " + txp.getNetworkName() );
  var bc = this._getBlockchainExplorer(txp.getNetworkName()); //correct
...
WalletService._clearBlockchainHeightCache = function(network) {
  ...
  if (!_.contains(['livenet', 'testnet','bcclivenet', 'bcctestnet',], network)) {
...
WalletService.prototype._getBlockchainHeight = function(network, cb) {
  ...
  function fetchFromBlockchain(cb) {
    console.log("WalletService.prototype._getBlockchainHeight: " + network + "=" + self.network);
    var bc = self._getBlockchainExplorer(network);
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function getNormalizedTxs(addresses, from, to, netname, cb) {
...
//        console.log("getNormalizedTxs: " + network + "=" + netname);
        var bc = self._getBlockchainExplorer(netname); //correct
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.storage.fetchAddresses(self.walletId, function(err, addresses) {
      ...
      async.waterfall([
        ...
        function(next) {
          getNormalizedTxs(addresses, from, to, wallet.network , next);
        },
        function(txs, next) {
...
WalletService.prototype.scan = function(opts, cb) {
  ...
  function checkActivity(address, network, cb) {
    ...
    console.log("checkActivity: " + network + "=" + self.network);
    var bc = self._getBlockchainExplorer(network);
